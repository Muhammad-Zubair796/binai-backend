...
# DEBUG: List available models for the first key
if google_keys:
    try:
        print(f"DEBUG: Scanning available models for Key #1...", flush=True)
        genai.configure(api_key=google_keys[0])
        for m in genai.list_models():
            if 'generateContent' in m.supported_generation_methods:
                print(f"DEBUG: Found Google Model: {m.name}", flush=True)
    except Exception as e:
        print(f"DEBUG: Model Scan Failed: {str(e)}", flush=True)
...
def call_vision_model(prompt, image_bytes):
    """Official Google Library -> OpenRouter -> Groq."""
    
    # 1. Try Google Gemini (Using Official Library to fix 404/v1beta)
    if google_keys:
        for i in range(len(google_keys)):
            ...
            try:
                ...
                # Convert bytes to PIL Image for the official library
    ...
    # 2. Try OpenRouter (Using a more stable free slug)
    openrouter_key = os.getenv("OPENROUTER_API_KEY")
    if openrouter_key:
        try:
            print("DEBUG: Trying OpenRouter (gemini-2.0-flash-001)...", flush=True)
    ...
    # 3. Try Groq (Using the only remaining vision model)
